...
const int sleepTime = 1800; // 1800 seconds = 30 minutes

#define FINAL_SLEEP_INTERVAL 4 // 4 * sleepTime = 2 hours
...
void setup() {
  ...
  if (CURRENT_SLEEP_INTERVAL == FINAL_SLEEP_INTERVAL) {
    blink(5);
    resetSleepInterval();
    doSomething(); // do something after 2 hours only
    goToSleep();
  } else {
    blink(5);
...
void resetSleepInterval() {
  ...
  Serial.print("[INFO] Reset sleep interval: ");
...
void increaseSleepInterval() {
  ...
  Serial.print("[INFO] Increase sleep interval: ");
  Serial.println(CURRENT_SLEEP_INTERVAL);
}
